...
用户唤醒 leader（输入框消息）；leader/队友经 hub 唤醒子代理
（asyncio.Event）；子代理永远不能唤醒任何上游。leader 被中断
（Esc）时团队不散伙：调度器置 pause 标志，running 子代理在步边界
挂起（不杀正在执行的工具），交付持续落 hub；leader 的下一条用户
消息进入新一轮，首个步边界注入积压交付，pause 清除并按需恢复
parked 子代理。**团队寿命 > 单回合**。
